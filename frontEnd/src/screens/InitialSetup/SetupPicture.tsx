...
export const SetupPicture = ({
  ...
  const saveImagePickerResult = async (
  ...
  const onSubmitPress = () => {
    ...
    setup(store.getState()).catch((err: Error) => {
      Alert.alert("Error", err.message);
    });
  };

  const handleCamera = () => {
    ImagePicker.launchCameraAsync(imagePickerSettings)
      .then(saveImagePickerResult)
      .catch((err: Error) => Alert.alert("Error", err.message));
  };

  const handleGallery = () => {
    ImagePicker.launchImageLibraryAsync(imagePickerSettings)
      .then(saveImagePickerResult)
      .catch((err: Error) => Alert.alert("Error", err.message));
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.button}>
          <Button
            title={"I'm fine with this picture!"}
