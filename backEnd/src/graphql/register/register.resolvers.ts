...
export default {
    Mutation: {
        register: async (_: any, data: { input: { username: string, email: string, password: string } }, _2: any) => {
            const { username, email, password } = data.input

            if (await checkUsernameExists(username) || await checkEmailExists(email)) {
    ...
    Query: {
        checkUsernameExists: async (_: any, data: { username: string }, _2: any) => {
            const { username } = data
