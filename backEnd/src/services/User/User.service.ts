import { UserModel, IUser, HobbyModel } from "../../models";
...
const randomIndex = (maxLen: number) => Math.floor(Math.random() * maxLen);
...
export const addMock = async () => {
  ...
  for (let i = 0; i < 50; ++i) {
    ...
    let randomHobbiesIndexes = [];

    while (randomHobbiesIndexes.length < 5) {
      const idx = randomIndex(hobbiesLength);

      if (!randomHobbiesIndexes.includes(idx)) randomHobbiesIndexes.push(idx);
    }

    let userHobbies = [];

    for (let i = 0; i < 5; ++i)
      userHobbies.push(hobbies[randomHobbiesIndexes[i]]._id);
